...
async function editTitle(data) {
    return await axios({
        method: 'POST',
        url: `https://bugs.telegram.org/api?hash=${data.hash}&issue_id=${data.url_id}&title=${encodeURIComponent(data.title)}&description=${encodeURIComponent(
            data.description
        )}&method=editIssue`,
        headers: {
            Accept: 'application/json, text/javascript, */*; q=0.01',
...
async function loadComments(data) {
    return await axios({
        ...
        .then((response) => {
            if (response.data.error !== undefined) return { comments: [], url_id: Number(data.url_id), after_id: Number(data.after_id) };

            // The best kludge to get rid of replies.
            if (
                response.data.comments_html.match(/<a href="(.*)" data-layer class="bt-comment-reply-content" data-comment-link="(.*)">((.|\n)*?)<\/a>/g) !==
                null
            ) {
                response.data.comments_html = response.data.comments_html.replace(
                    /<a href="(.*)" data-layer class="bt-comment-reply-content" data-comment-link="(.*)">((.|\n)*?)<\/a>/,
                    ''
                );
            }
            ...
            for (let i = 0; i < response.data.comments_html.match(/<span class="bt-comment-author-name">(.*)<\/span>/gi).length; i++) {
                comments.push({
                    ...
                    comment_id: response.data.comments_html
                        .match(/<div class="bt-comment" data-comment-id="(.*)">/gi)
                        [i].replace(/<div class="bt-comment" data-comment-id=|>|['"]+/g, ''),
                });
            }
...
async function getMe(data) {
    return await axios({
        ...
        .then((response) => {
            const suggestionCount = response.data.h
                .match(/<h3 class="bt-header" data-count="(.+)">/gi)[0]
                .match(/data-count="(.+)"/)[0]
                .replace(/data-count=|['"]+/gi, '');
            const username = response.data.h
                .match(/<span class="bt-dropdown-item">(.+)<\/span>/gi)[0]
                .split('<li>')[0]
                .replace(/<\/?[^>]+(>|$)/g, '');
