...
module.exports = () => async (ctx) => {
    try {
        ...
        if (ctx.message.text.match(/^\/start|\/me|\/new|\/suggest$/gi) !== null)
            return replyWithError(ctx, 16);
        if (ctx.message.text.length <= config.description_minimum_length)
            return replyWithError(ctx, 15);
        ...
        ctx.session.newCard.entities =
            ctx.message.entities === undefined ? null : ctx.message.entities;
